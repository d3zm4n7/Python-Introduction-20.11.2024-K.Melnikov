...
def validate_data():
    ...
    tk.messagebox.showinfo("Edu", "Andmed on valideeritud!")
    clear_entries()
    return True
...
def insert_data(entries):
    if validate_data():
        ...
        print("Sisestatud andmed:", entries["Pealkiri"].get(), entries["Aasta"].get())
...
# Loo sildid ja sisestusväljad
    labels = ["Pealkiri", "Režissöör", "Aasta", "Žanr", "Kestus", "Reiting", "Keel", "Riik", "Kirjeldus"] # лэйблы
    entries = {}

    for i, label in enumerate(labels): # enumirate пронумировывает лэйблы
...
# Loo nupp andmete sisestamiseks
    submit_button = tk.Button(root, text="Sisesta andmed", command=lambda: insert_data(entries))
